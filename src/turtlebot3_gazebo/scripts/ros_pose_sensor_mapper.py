...
class PoseSensorMapper:
    ...
    def __init__(self):
        self.pose_sub = message_filters.Subscriber("/odom", Odometry)
        self.laser_sub = message_filters.Subscriber("/scan", LaserScan)
        self.image_sub = message_filters.Subscriber("/camera/rgb/image_raw", Image)

        self.ts = message_filters.ApproximateTimeSynchronizer([self.pose_sub, self.laser_sub, self.image_sub], 10, 1)
        self.ts.registerCallback(self.callback)
        
        self.bridge = CvBridge()

    def callback(self, pose_msg, laser_msg, image_msg):
        ## POSE
